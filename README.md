# kayedejour'

A personal project designed for daily journaling and private use.

## About

This application was built as a personal project to store daily journals and for private usage. It provides a simple, secure way to document daily life and thoughts.

## Features

- **Daily Journaling**: Dedicated interface for tracking daily activities.
- **Rich Text Editor**: Full formatting support for expressive entries.
- **Image Support**: Upload and optimize images for your journals.
- **Location Tracking**: Automatically tag entries with location context.
- **Responsive Design**: Seamless experience across desktop and mobile.

## Tech Stack

- **Frontend**: React, TypeScript, Vite
- **Styling**: TailwindCSS
- **Backend**: Supabase

## Getting Started

1.  Clone the repository:

    ```bash
    git clone https://github.com/yourusername/kayedejour.git
    ```

2.  Install dependencies:

    ```bash
    npm install
    ```

3.  Run the development server:
    ```bash
    npm run dev
    ```
