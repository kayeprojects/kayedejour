...
import { Folder, Plus, Book, Trash2, LogOut, User as UserIcon, ChevronDown, X } from 'lucide-react'
...
interface SidebarProps {
  ...
  onLogin: () => void
  onLogout: () => void
  /** Mobile: whether sidebar overlay is open */
  isMobileOpen?: boolean
  /** Mobile: callback to close the sidebar overlay */
  onMobileClose?: () => void
}

export function Sidebar({ activeFolder, setActiveFolder, folders, onCreateFolder, onDeleteFolder, user, onLogin, onLogout, isMobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const handleFolderSelect = (folder: string) => {
    ...
    // Auto-close sidebar on mobile when selecting a folder
    if (onMobileClose) {
      onMobileClose()
    }
  }
  ...
  return (
    <>
      ...
      <aside 
        className={cn(
          "bg-gray-50 dark:bg-gray-900 flex flex-col transition-all duration-300 h-screen shrink-0 z-40 border-r border-gray-200 dark:border-gray-800",
          // Desktop: static sidebar
          "lg:relative lg:translate-x-0 lg:w-64 lg:sticky lg:top-0",
          // Mobile: fixed overlay
          "fixed inset-y-0 left-0 w-[280px] sm:w-80",
      ...
      >
        ...
        <button
          onClick={onMobileClose}
          className="absolute top-4 right-4 p-2 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 hover:text-gray-900 dark:hover:text-white lg:hidden z-50"
        ...
        <nav className="flex-1 px-3 lg:px-4 space-y-1 overflow-y-auto">
          ...
          <button
            onClick={() => handleFolderSelect('All Notes')}
            className={cn(
              "w-full flex items-center gap-3 px-3 py-2.5 lg:py-2 rounded-md text-sm transition-all duration-200",
          ...
          {isCreatingFolder ? (
            <form onSubmit={handleCreateSubmit} className="px-3 py-2">
              <input
                ...
                className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-gray-400 dark:text-white"
              />
            </form>
          ) : (
            <Button
              ...
              className="w-full justify-start gap-3 px-3 py-2.5 lg:py-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
            >
          ...
          {user ? (
            <>
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 lg:py-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
              >
